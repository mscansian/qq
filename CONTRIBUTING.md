...
- **Untrusted data is framed as data.** Stdin combined with an
  argument gets wrapped in a `<content-NONCE>...</content-NONCE>`
  tag where the nonce is 64 fresh bits per invocation, and the
  system prompt refers to that exact tag. A payload can't close
  the region without guessing the nonce, and as a secondary
  defense any literal collision with the nonce tag is escaped.
