...
Your stdin exceeded the 200 KiB default cap. `qq` proceeds with
...
File permissions on `credentials.toml` drifted. `qq` proceeds but
by the time you see this warning, the API key may already have
been read by another user on the machine. Re-chmod:
