...
- Stdin content piped to the model is wrapped in
  `<content>...</content>` tags, any literal `</content>` inside the
  payload is escaped so it can't close the region early, and the
  system prompt explicitly frames that region as untrusted data rather
  than instructions.
...
**What `qq` does.** Wraps stdin in `<content>...</content>`, escapes
the literal delimiter inside the payload so it can't be closed early,
and instructs the model in its system prompt to treat the tagged
region as data, not instructions.

**What's still on you.** The mitigation is partial. Model compliance
with "treat this as data" is probabilistic and varies across models.
Don't pipe content you don't trust into questions where the *shape*
of the answer matters, and read model output critically before acting
on it.
